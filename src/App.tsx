...
const questions: Question[] = [
  { number: '01', title: 'CONDUTA', prompt: 'Uma provocação surge no chat antes de um confronto. Como você reage?', answers: [
    ...
    { label: 'Aceito o desafio e deixo minha atuação falar por mim.', scores: { poder: 3, coragem: 2 } as Partial<Record<Attribute, number>> },
  ...
  { number: '02', title: 'DISCIPLINA', prompt: 'Você recebe uma ordem que não seria sua primeira escolha.', answers: [
    ...
    { label: 'Adapto a ordem ao meu estilo e entrego o resultado.', scores: { iniciativa: 3, poder: 2 } as Partial<Record<Attribute, number>> },
  ...
  { number: '03', title: 'PRESENÇA', prompt: 'Quando você chega em um grupo novo, qual é sua postura?', answers: [
    ...
    { label: 'Me apresento e encontro rapidamente uma função.', scores: { liderança: 3, iniciativa: 2 } as Partial<Record<Attribute, number>> },
  ...
  { number: '04', title: 'EQUIPE', prompt: 'Uma parte do time está atrasada para o evento.', answers: [
    ...
    { label: 'Assumo uma função extra para ganhar tempo.', scores: { poder: 2, iniciativa: 3 } as Partial<Record<Attribute, number>> },
  ...
  { number: '05', title: 'CONFIANÇA', prompt: 'O que sustenta uma gangue forte?', answers: [
    ...
    { label: 'A coragem de continuar quando o cenário muda.', scores: { poder: 3, iniciativa: 2 } as Partial<Record<Attribute, number>> },
  ] },
  { number: '06', title: 'CONFLITO', prompt: 'Dois membros discordam antes de uma decisão importante.', answers: [
    ...
    { label: 'Proponho um teste rápido para decidir com evidência.', scores: { estratégia: 4, iniciativa: 1 } as Partial<Record<Attribute, number>> },
  ...
  { number: '07', title: 'CORAGEM', prompt: 'Qual é a sua definição de coragem?', answers: [
    { label: 'Entrar em ação mesmo quando o plano não é perfeito.', scores: { poder: 3, iniciativa: 2 } as Partial<Record<Attribute, number>> },
  ...
  { number: '08', title: 'ESTRATÉGIA', prompt: 'O plano original deixa de funcionar no meio da missão.', answers: [
    { label: 'Improviso uma rota e mantenho o objetivo intacto.', scores: { estratégia: 4, iniciativa: 1 } as Partial<Record<Attribute, number>> },
  ...
  { number: '09', title: 'RESPONSABILIDADE', prompt: 'Você percebe que cometeu um erro.', answers: [
    ...
    { label: 'Reparo o que for possível antes de chamar atenção.', scores: { iniciativa: 3, disciplina: 2 } as Partial<Record<Attribute, number>> },
  ...
  { number: '10', title: 'COMPETITIVIDADE', prompt: 'O que uma derrota muda em você?', answers: [
    ...
    { label: 'Volto mais forte e procuro uma nova oportunidade.', scores: { poder: 3, iniciativa: 2 } as Partial<Record<Attribute, number>> },
  ...
  { number: '11', title: 'INICIATIVA', prompt: 'Não existe uma tarefa definida para você.', answers: [
    { label: 'Encontro uma lacuna e proponho uma solução.', scores: { iniciativa: 4, liderança: 1 } as Partial<Record<Attribute, number>> },
    ...
    { label: 'Me junto à função que parece mais exigente.', scores: { poder: 3, coragem: 2 } as Partial<Record<Attribute, number>> },
  ] },
  { number: '12', title: 'LEALDADE', prompt: 'Um amigo pede para você ignorar uma regra do servidor.', answers: [
    ...
    { label: 'Procuro uma alternativa permitida para ajudar.', scores: { estratégia: 3, iniciativa: 2 } as Partial<Record<Attribute, number>> },
    { label: 'Recuso, mesmo que isso gere uma conversa difícil.', scores: { poder: 2, lealdade: 3 } as Partial<Record<Attribute, number>> },
  ] },
  { number: '13', title: 'LIDERANÇA', prompt: 'O grupo precisa de direção, mas ninguém se manifesta.', answers: [
    ...
    { label: 'Começo a agir e crio movimento pelo exemplo.', scores: { iniciativa: 3, poder: 2 } as Partial<Record<Attribute, number>> },
  ...
  { number: '14', title: 'CONTROLE', prompt: 'Uma situação começa a sair do controle durante a call.', answers: [
    ...
    { label: 'Interrompo o ruído e tomo uma decisão rápida.', scores: { poder: 3, iniciativa: 2 } as Partial<Record<Attribute, number>> },
  ...
  { number: '15', title: 'TRAJETÓRIA', prompt: 'O que você procura ao entrar na Allied?', answers: [
    ...
    { label: 'Um grupo onde presença e competência sejam reconhecidas.', scores: { poder: 2, iniciativa: 3 } as Partial<Record<Attribute, number>> },
    ...
    <div className={`allied-app ${audioOn ? 'ambient-on' : ''}`}>
      ...
      <header className={`site-nav ${scrolled ? 'nav-scrolled' : ''}`}>
        ...
        <nav className={menuOpen ? 'nav-links nav-links-open' : 'nav-links'}>
          {['home:HOME', 'allied:ALLIED', 'hierarchy:HIERARQUIA', 'stats:ESTATÍSTICAS', 'rules:REGRAS', 'test:TESTE', 'faq:FAQ'].map((link) => { const [id, label] = link.split(':'); return <a href={`#${id}`} key={id} onClick={closeMenu}>{label}</a>; })}
      ...
      <main>
        <section className="hero" id="home">
          ...
          <div className="hero-content reveal">
            <SectionLabel number="01 / 07">GAKURAN • PT-BR</SectionLabel>
        ...
        <section className="manifesto section-dark" id="allied">
          <div className="content-grid">
            <div className="reveal"><SectionLabel number="02 / 07">A ALLIED</SectionLabel><h2>Não é apenas<br /><span>entrar.</span></h2><p className="large-copy">A Allied é uma gangue completa dentro de Gakuran, construída sobre hierarquia, disciplina, competitividade e presença. Nossa estrutura existe para transformar jogadores em membros preparados para representar a gangue dentro e fora dos confrontos.</p><a className="text-link" href="#hierarchy">CONHEÇA NOSSA ESTRUTURA <ArrowRight size={16} /></a></div>
        ...
        <section className="central section-paper" id="central">
          <div className="section-heading reveal"><div><SectionLabel number="03 / 07">PORTAS DE ACESSO</SectionLabel><h2>Central da <span>Allied</span></h2></div><p>Três destinos. Uma mesma origem.<br />Escolha onde sua história começa.</p></div>
        ...
        <section className="hierarchy section-dark" id="hierarchy">
          <div className="content-grid hierarchy-grid"><div className="reveal"><SectionLabel number="04 / 07">A ESTRUTURA</SectionLabel><h2>Hierarquia<br /><span>ativa.</span></h2><p className="large-copy">A hierarquia define a estrutura. Cada posição exige presença, responsabilidade e a vontade de proteger o nome Allied.</p><div className="vertical-note"><span>HIERARQUIA / ATIVA</span><i /></div></div><div className="leader-stack"><article className="leader-card leader-primary reveal"><div className="leader-photo leader-one" /><div className="leader-info"><span>LÍDER / 01</span><h3>Toru</h3><p>O ponto de direção. A voz que mantém a estrutura em movimento.</p></div><LogoMark small /></article><article className="leader-card reveal"><div className="leader-photo leader-two" /><div className="leader-info"><span>SUB-LÍDER / 02</span><h3>Maniack</h3><p>Presença de apoio, disciplina em cada decisão.</p></div><LogoMark small /></article></div></div>
        </section>

        <section className="stats section-paper" id="stats"><div className="section-heading reveal"><div><SectionLabel number="05 / 07">REGISTRO ALLIED</SectionLabel><h2>Uma estrutura<br /><span>em movimento.</span></h2></div><span className="stamp">ALLIED<br />RECORDS</span></div><div className="stats-grid">{stats.map(({ value, label, note, icon: Icon, featured }, index) => <div className={`stat-card reveal ${featured ? 'stat-featured' : ''}`} key={label} style={{ transitionDelay: `${index * 70}ms` }}><Icon size={17} /><strong>{value}</strong><h3>{label}</h3><p>{note}</p><span className="stat-index">0{index + 1}</span></div>)}</div></section>

        <section className="rules section-dark" id="rules"><div className="rules-intro reveal"><SectionLabel number="06 / 07">CÓDIGO DA ALLIED</SectionLabel><h2>As regras existem<br />para preservar<br /><span>nossa estrutura.</span></h2><p>Um nome forte exige uma conduta à altura. Leia antes de entrar.</p></div><div className="rules-list">{rules.map(([title, description], index) => <article className="rule reveal" key={title}><span className="rule-number">{String(index + 1).padStart(2, '0')}</span><div><h3>{title}</h3><p>{description}</p></div><Check size={15} /></article>)}</div></section>

        <section className="quiz-section section-paper" id="test"><div className="quiz-intro reveal"><div><SectionLabel number="07 / 07">入隊試験 / RECRUITMENT TEST</SectionLabel><h2>Descubra sua<br /><span>divisão.</span></h2><p>Quinze perguntas. Quatro caminhos em cada uma. Nenhuma resposta é certa — apenas revela como você se posiciona.</p><button className="button button-red" onClick={startQuiz}>FAZER O TESTE <ArrowRight size={16} /></button></div><div className="quiz-seal"><span>入隊</span><small>ALLIED<br />EXAM / 001</small></div></div></section>

        <section className="faq section-dark" id="faq"><div className="section-heading reveal"><div><SectionLabel>よくある質問</SectionLabel><h2>Perguntas<br /><span>frequentes.</span></h2></div><p>Antes de entrar, conheça o lugar<br />que você está prestes a ocupar.</p></div><div className="faq-list">{['Como entro na Allied?', 'Preciso jogar bem para entrar?', 'A Allied é uma gangue do Gakuran?', 'Preciso estar no Discord?', 'Existem eventos e tryouts?', 'Posso entrar mesmo sendo iniciante?', 'Existe hierarquia?', 'Como descubro minha divisão?'].map((question, index) => { const answersText = ['Abra um ticket no Discord da Allied e envie qualquer mensagem. A equipe irá orientar você.', 'Não. A evolução dentro da Allied acontece com presença, disciplina e participação.', 'Sim. A Allied é uma gangue completa dentro do universo de Gakuran.', 'Sim. O Discord é o principal espaço de comunicação e organização da Allied.', 'Sim. Eventos, treinos, tryouts e atividades podem ser realizados de acordo com a organização da equipe.', 'Sim. A Allied possui espaço para membros iniciantes. O importante é disposição para participar e evoluir.', 'Sim. A Allied possui uma estrutura hierárquica para organização, disciplina e funcionamento interno.', 'Realize o teste de recrutamento disponível no site.']; return <div className="faq-item reveal" key={question}><button type="button" aria-expanded="true" aria-controls={`faq-answer-${index}`}><span>0{index + 1}</span><strong>{question}</strong><ChevronDown size={17} /></button><div className="faq-answer" id={`faq-answer-${index}`}><p>{answersText[index]}</p></div></div>; })}</div></section>
      ...
      <footer className="site-footer"><div className="footer-brand"><LogoMark small /><div><strong>ALLIED</strong><span>GAKURAN • PT-BR</span></div></div><p>Uma gangue completa dentro do universo de Gakuran.</p><div className="footer-links"><a href="https://www.roblox.com/pt/games/128736949265057/Gakuran" target="_blank" rel="noreferrer">ROBLOX</a><a href="https://discord.gg/gakuran" target="_blank" rel="noreferrer">DISCORD GAKURAN</a><a href="https://discord.gg/UFUMMx5PkD" target="_blank" rel="noreferrer">DISCORD ALLIED</a></div><button className="audio-toggle" onClick={() => setAudioOn((on) => !on)} aria-label="Ativar ambiente visual"><AudioLines size={15} /> {audioOn ? 'AMBIENTE ON' : 'AMBIENTE OFF'}</button><span className="copyright">© ALLIED / 2026</span></footer>

      {quizOpen && <div className="quiz-modal" role="dialog" aria-modal="true" aria-label="Teste de recrutamento"><div className="quiz-window">{result ? <div className="result-screen"><button className="quiz-close" onClick={closeQuiz}><X size={18} /></button><div className="result-orbit"><LogoMark /><span>{result.division.split('ª')[0]}ª</span></div><SectionLabel>RESULTADO DO TESTE</SectionLabel><p className="result-eyebrow">{divisions[result.division].eyebrow}</p><h2>{result.name},<br /><span>{result.division}</span></h2><p className="result-title">{divisions[result.division].title}</p><p className="result-description">{divisions[result.division].description}</p><div className="score-grid">{attributeKeys.map((key) => <div className="score-row" key={key}><span>{attributeLabels[key]}</span><div><i style={{ width: `${result.scores[key]}%` }} /></div><strong>{result.scores[key]}%</strong></div>)}</div><div className="result-actions"><a className="button button-red" href="https://discord.gg/UFUMMx5PkD" target="_blank" rel="noreferrer">ENTRAR NA ALLIED <ArrowUpRight size={15} /></a><button className="button button-ghost" onClick={startQuiz}>REFAZER TESTE</button></div></div> : <><div className="quiz-header"><div><span>入隊試験</span><strong>TESTE DE RECRUTAMENTO</strong></div><button className="quiz-close" onClick={closeQuiz}><X size={18} /></button></div><div className="quiz-progress"><span>PERGUNTA {String(quizStep + 1).padStart(2, '0')} / {questions.length}</span><div><i style={{ width: `${Math.max(7, progress)}%` }} /></div><span>{progress}%</span></div><div className="quiz-body"><span className="question-number">{currentQuestion.number}</span><p className="question-category">{currentQuestion.title}</p><h2>{currentQuestion.prompt}</h2><div className="answers">{currentQuestion.answers.map((answer, index) => <button className={answers[quizStep] === index ? 'answer selected' : 'answer'} key={answer.label} onClick={() => selectAnswer(index)}><span>{String.fromCharCode(65 + index)}</span><strong>{answer.label}</strong>{answers[quizStep] === index && <Check size={16} />}</button>)}</div><div className="quiz-footer"><span>DISCIPLINA · LEALDADE · PRESENÇA</span><div>{quizStep > 0 && <button className="back-button" onClick={() => setQuizStep((step) => step - 1)}>VOLTAR</button>}<button className="button button-red" disabled={answers[quizStep] === undefined} onClick={() => quizStep === questions.length - 1 ? finishQuiz() : setQuizStep((step) => step + 1)}>{quizStep === questions.length - 1 ? 'REVELAR DIVISÃO' : 'PRÓXIMA' } <ArrowRight size={15} /></button></div></div></div></>}</div></div>}
    </div>
  );
}
